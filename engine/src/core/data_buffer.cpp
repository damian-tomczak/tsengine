...
namespace ts
{
DataBuffer::DataBuffer(const Context& context) : mContext{context}
{}
...
void DataBuffer::createDataBuffer(VkBufferUsageFlags bufferUsageFlags, VkMemoryPropertyFlags memoryProperties, VkDeviceSize size)
{
    ...
    if (!khronos_utils::findSuitableMemoryTypeIndex(mContext.getVkPhysicalDevice(), memoryRequirements, memoryProperties,
...
{
    void* data;
    LOGGER_VK(vkMapMemory, mContext.getVkDevice(), mDeviceMemory, 0, mSize, 0, &data);
...
{
    vkUnmapMemory(mContext.getVkDevice(), mDeviceMemory);
}
}