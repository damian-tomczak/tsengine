...
private:
    ...
    virtual void call(Event& e) override { std::invoke(mCallback, mpOwner, static_cast<TEvent&>(e)); }
};
...
public:
    ...
    void subscribeToEvent(TOwner* pOwner, void (TOwner::*callback)(TEvent&))
    {
        ...
        mSubscribers.at(typeid(TEvent))->emplace_back(std::make_unique<EventCallback<TOwner, TEvent>>(pOwner, callback));
    }
